...
export class ViewDrinkOrderComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.intervalId = setInterval(() => {
      if (document.visibilityState === 'visible') {
        console.log("🕒 Auto-refreshing drink orders...");
        this.loadHeldOrders();
        this.loadProcessedOrders();
      }
    }, 80000);
  ...
  /**
   * Load held drink orders from the server
   */
  async loadHeldOrders() {
    try {
      ...
      if (res && Array.isArray(res)) {
        ...
        this.applyFilters();
        console.log("✅ Drink Orders Loaded:", this.itemList.length, "orders");
      }
    } catch (error) {
      console.error("❌ Error loading drink orders:", error);
      this.toastr.error('Failed to load drink orders', 'Error');
    } finally {
  ...
  /**
   * Load processed drink orders from the server
   */
  async loadProcessedOrders() {
    try {
      ...
      if (res && Array.isArray(res)) {
        ...
        console.log("✅ Processed Drink Orders Loaded:", this.processedOrders.length, "orders");
      }
    } catch (error) {
      console.error("❌ Error loading processed drink orders:", error);
      this.toastr.error('Failed to load processed drink orders', 'Error');
    } finally {
      this.cdr.detectChanges();
    }
  }
  ...
  getStatusText(order: any): string {
    if (order.large_format_status === 'yes') {
      return 'Pending';
    } else if (order.large_format_status === 'no') {
      return 'Confirmed';
