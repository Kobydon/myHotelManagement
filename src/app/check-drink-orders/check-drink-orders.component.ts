...
@Component({
  selector: 'check-drink-orders',
  templateUrl: './check-drink-orders.component.html',
  styleUrls: ['./check-drink-orders.component.css']
})
export class CheckDrinkOrdersComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  async searchDates() {
    try {
      ...
      const [
        ...
        this.guestService.searchMostOrderedDateDrink(d),
  ...
  loadHeldOrders() {
    const selectedDate = this.paymentForm.value.dates;
    this.guestService.getHeldReportOrdersDrink(selectedDate).subscribe((data) => {
