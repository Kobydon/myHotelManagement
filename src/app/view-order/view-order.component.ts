...
export class ViewOrderComponent implements OnInit, OnDestroy {
  ...
  /**
   * Load held orders from the server
   */
  async loadHeldOrders() {
    try {
      ...
      if (res && Array.isArray(res)) {
        ...
        this.applyFilters();
        console.log("✅ Incoming Orders Loaded:", this.itemList.length, "orders");
      }
    } catch (error) {
      console.error("❌ Error loading incoming orders:", error);
      this.toastr.error('Failed to load incoming orders', 'Error');
    } finally {
  ...
  /**
   * Load processed orders from the server
   */
  async loadProcessedOrders() {
    try {
      const res = await this.guestService.getHeldaOrders();
      ...
      if (res && Array.isArray(res)) {
        ...
        console.log("✅ Processed Orders Loaded:", this.processedOrders.length, "orders");
      }
    } catch (error) {
    ...
    if (printWindow) {
      let receiptContent = `
        ...
        <body onload="window.print(); window.close();">
          ...
          <div class="info"><strong>🧾 ORDER CONFIRMATION</strong></div>
  ...
  getStatusText(order: any): string {
    if (order.digital_printing_status === 'yes') {
      return 'Pending';
    } else if (order.digital_printing_status === 'no') {
      return 'Confirmed';
