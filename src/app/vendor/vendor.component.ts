...
export class VendorComponent implements OnInit {
  ...
  async saveVendor(itemData: any) {
    try {
      this.loading.start();
      let res;
      if (this.header === 'Add New') {
        res = await this.guestService.addVendor(itemData); // Assuming addItem() adds a new item
        this.toastr.success('Item successfully added');
      } else if (this.header === 'Edit') {
        res = await this.guestService.updateVendor(itemData); // Assuming updateItem() updates an existing item
        this.toastr.success('Item successfully updated');
      }
      if (res) {
        this.getVendorList(); // Refresh the item list
  ...
  // Delete item
  async deleteVendor(itemId: number) {
    try {
      this.loading.start();
      const res = await this.guestService.deleteVendor(itemId); // Assuming deleteItem() deletes an item
      if (res) {
