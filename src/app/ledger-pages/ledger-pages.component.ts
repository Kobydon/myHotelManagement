 ...
 returnList:any;
  ...
  async searchDates() {
    try {
      ...
      const z = { date: selectedDate ,datetwo:this.paymentForm.value.date_two};
  
      // Fetch data
      const paymentRes = await this.paymentService.searchDatesTwo(d);
      this.paymentList = paymentRes || [];
      this.totalAmount = this.paymentList.reduce((sum, item) => sum + parseInt(item.amount), 0);
  
      const refundRes = await this.paymentService.searchRefundDatesTwo(d);
      this.refundList = refundRes || [];
      this.totalRefundAmount = this.refundList.reduce((sum, item) => sum + parseInt(item.refund_amount), 0);
  
      const incomeRes = await this.guestService.searchIncomeDatesTwo(z);
      this.incomeList = incomeRes || [];
      this.totalIncome = this.incomeList.reduce((sum, item) => sum + parseInt(item.amount), 0);
  
      const expenseRes = await this.guestService.searchExpenseDateTwo(z);
      this.expenseList = expenseRes || [];
      this.totalExpenses = this.expenseList.reduce((sum, item) => sum + parseInt(item.amount), 0);
  
    const purchaseRes = await this.guestService.searchPurchaseDateTwo(d);
      this.purchaseList = purchaseRes || [];
      this.totalPurchases = this.purchaseList.reduce((sum, item) => sum + parseInt(item.total_cost), 0);
  
      // Compute the balance after all debits and credits
  ...
  exportexcel()
  {
    ...
    let element = document.getElementById('ledger-table');
