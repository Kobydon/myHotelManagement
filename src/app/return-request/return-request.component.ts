  ...
  ngOnInit(): void {
   ...
   async getUser(){
    ...
    async getReturnList() {
      try {
        // this.loading.start();
        const res = await this.guestService.getReturnRequest(); // Assuming getItemsList() fetches the items from the API
        if (res) {
          this.returnList = res;
        }
      } catch (error) {
   ...
   // Refresh the item list
    ...
    async approve(itemData: any) {
      try {
        this.loading.start();
        let res;
        res = await this.guestService.approveReturnRequest(itemData);
        if (res) {
         // Refresh the item list
          this.getReturnList();// Close the modal
        }
      } catch (error) {
        this.toastr.error('Error saving item');
      } finally {
