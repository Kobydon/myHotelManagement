...
constructor(
  ...
  async getTodo(){
    try{
      this.loading.start();
      var res = await this.employeeService.getTodo();
      if(res) this.todoList=res; 
  ...
  try{
    this.loading.start();
   var res  = await this.employeeService.AddTodo(emp);
   if(res)this.toastr.success(null,"Attendance successfully added"); this.closePopup();this.getTodo();
  ...
  try{
    this.loading.start();
    this.emmp_info =  await this.employeeService.getTodoDetails(id);
  
    if ( this.emmp_info)  
    this.createForm.patchValue({
     id:this.emmp_info[0].id,
      name:this.emmp_info[0].name,
      position:this.emmp_info[0].position,
      description:this.emmp_info[0].description,
  ...
  try{
    this.loading.start();
   var res  = await this.employeeService.updateTodo(emp);
   if(res) this.closePopup();this.getTodo();
  ...
  this.loading.start();
  var res = await this.employeeService.deleteTodo(id);
  if(res) this.getTodo();
