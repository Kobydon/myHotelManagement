  ...
  constructor(private fb:FormBuilder,private toastr:ToastrService,private guestService:GuestService,
    ...
    async searchDates(){
      ...
      const d = {
        ...
        datetwo: this.createForm.value.datetwo
      }
        try{
          this.loading.start();
          this.searchIncomeDates(d.date,d.datetwo);
          this.searchExpenseDates(d.date,d.datetwo);
    ...
    async calculateFinancialOverview() {
      // Calculate the total GOP Deduction
      this.gopDeduction = this.gopList.reduce((sum, item) => sum + parseInt(item.amount), 0);
  
      // Calculate the NOP (Net Operating Profit)
      this.nop = (this.totalIncome - this.totalExpenditure) - this.gopDeduction;
  
      // Calculate Cash at Hand
      this.cashAtHand = this.totalIncome - this.totalExpenditure - this.gopDeduction;
    }
  
    async searchIncomeDates(data, datetwo) {
      let sum: number = 0;
      const d = {
        date: data,
        datetwo: datetwo
      ...
      try {
        this.loading.start();
        const res = await this.guestService.searchIncomeDatesTwo(d);
        ...
        // Recalculate financial overview after income is fetched
    ...
    async searchExpenseDates(data, datetwo) {
      let sum: number = 0;
      const d = {
        date: data,
        datetwo: datetwo
      ...
      try {
        this.loading.start();
        const res = await this.guestService.searchExpenseDateTwo(d);
        ...
        // Recalculate financial overview after expenditure is fetched
        this.calculateFinancialOverview();
      } catch (err) {
        this.toastr.error(null, err.message);
      } finally {
    ...
    async getgopList() {
      try {
        this.loading.start();
        const res = await this.guestService.getgopList();
        if (res) {
          this.gopList = res;
        }
        // Recalculate financial overview after GOP list is fetched
        this.calculateFinancialOverview();
      } catch (error) {
